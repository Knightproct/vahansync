...
import './styles.css'
import { acceptInvitation, changeSubscription, completeComponentService, createComponent, createDocument, createExpense, createFuelTransaction, createInventoryMovement, createInventoryTransaction, createMaintenancePlan, createPart, createPurchaseOrder, createStockLocation, createSubscriptionCheckout, createTelematicsDevice, createTelematicsIntegration, createTollTransaction, createUser, createVehicle, createInvitation, createVendor, createWorkOrder, dispatchQueuedSms, downloadFile, exportResource, getComponents, getCurrentUser, getDocuments, getExpenses, getInvitations, getMaintenancePlans, getNotificationDeliveries, getNotificationPreferences, getNotifications, getParts, getPurchaseOrders, getStockLocations, getSubscription, getSubscriptionPlans, getTelematicsDevices, getTelematicsIntegrations, getUsers, getVehicles, getVendors, getWorkOrders, importResource, login, revokeInvitation, signupOrganization, syncDueTelematics, updateDocument, updateExpense, updateMyContact, updateNotification, updateNotificationPreference, updatePurchaseOrder, updateUserRole, updateVehicle, updateWorkOrder, uploadDocumentFile } from './api'
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page">
          ...
          {active === 'costs' && <Costs token={token} expenses={expenses} vehicles={fleet} onNotify={notify} onExpenseCreated={(expense) => setExpenses((current) => [expense, ...current])} />}
...
function Maintenance({ role, workOrders, components, plans, vehicles: fleet, token, onCreated, onUpdated, onPlanCreated, onNotify }) {
  ...
  return <div><PageHeader eyebrow="Workshop control" title="Maintenance" subtitle="Plan preventive care and close every work order on time." /><section className="panel table-panel"><div className="table-header"><div><h2>Create work order</h2><span>Driver defects and fleet priorities enter the planner here.</span></div></div><form className="form-grid" onSubmit={submitOrder}><select required value={order.vehicle_id} onChange={(event) => setOrder({ ...order, vehicle_id: event.target.value })}><option value="">Vehicle</option>{fleet.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.reg}</option>)}</select><input required placeholder="Title" value={order.title} onChange={(event) => setOrder({ ...order, title: event.target.value })} /><input placeholder="Description" value={order.description} onChange={(event) => setOrder({ ...order, description: event.target.value })} /><select value={order.priority} onChange={(event) => setOrder({ ...order, priority: event.target.value })}><option>Low</option><option>Medium</option><option>High</option></select><select value={order.status} onChange={(event) => setOrder({ ...order, status: event.target.value })}><option>Open</option><option>In progress</option><option>Completed</option><option>Cancelled</option></select><input type="date" value={order.due_date} onChange={(event) => setOrder({ ...order, due_date: event.target.value })} /><input placeholder="Assigned technician" value={order.assigned_to} onChange={(event) => setOrder({ ...order, assigned_to: event.target.value })} /><button className="primary-button" type="submit">Create work order</button></form></section><section className="panel table-panel"><div className="table-header"><div><h2>Preventive maintenance plan</h2><span>Use odometer or calendar thresholds.</span></div></div><form className="form-grid" onSubmit={submitPlan}><select required value={plan.vehicle_id} onChange={(event) => setPlan({ ...plan, vehicle_id: event.target.value })}><option value="">Vehicle</option>{fleet.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.reg}</option>)}</select><input required placeholder="Plan name" value={plan.name} onChange={(event) => setPlan({ ...plan, name: event.target.value })} /><input type="number" placeholder="Interval km" value={plan.interval_km} onChange={(event) => setPlan({ ...plan, interval_km: event.target.value })} /><input type="number" placeholder="Interval days" value={plan.interval_days} onChange={(event) => setPlan({ ...plan, interval_days: event.target.value })} /><input type="number" placeholder="Next due km" value={plan.next_due_km} onChange={(event) => setPlan({ ...plan, next_due_km: event.target.value })} /><input type="date" value={plan.next_due_on} onChange={(event) => setPlan({ ...plan, next_due_on: event.target.value })} /><button className="primary-button" type="submit">Create plan</button></form></section><div className="metric-grid compact"><MetricCard label="Open work orders" value={workOrders.length} change="live" detail="assigned records" icon="◷" tone="orange" /><MetricCard label="In progress" value={workOrders.filter((item) => item.status === 'In progress').length} change="live" detail="active jobs" icon="⌁" tone="blue" /><MetricCard label="Tracked components" value={components.length} change="live" detail={`${plans.length} service plans`} icon="◒" tone="green" /><MetricCard label="Preventive compliance" value="Live" change="odometer" detail="component thresholds" icon="✓" tone="purple" /></div><section className="panel table-panel"><div className="table-header"><div><h2>Work order planner</h2><span>{role === 'technician' ? 'Your assigned jobs' : 'All active and scheduled jobs'}</span></div></div><div className="data-table"><div className="data-row data-head"><span>Work order</span><span>Vehicle</span><span>Assigned to</span><span>Due</span><span>Priority</span><span>Status</span><span></span></div>{workOrders.map((item) => <div className="data-row" key={item.id}><div className="workorder-cell"><div className="maintenance-icon small amber">⌁</div><div><strong>{item.title}</strong><small>WO-{item.id}</small></div></div><span>{fleet.find((vehicle) => vehicle.id === item.vehicle_id)?.reg || 'Vehicle linked'}</span><span>{item.assigned_to || 'Assigned technician'}</span><span>{item.due_date || 'Unscheduled'}</span><span className={`priority ${item.priority === 'High' ? 'red' : item.priority === 'Low' ? 'green' : 'amber'}`}>{item.priority}</span><select value={item.status} onChange={async (event) => { try { const updated = await updateWorkOrder(token, item.id, { status: event.target.value }); onUpdated(updated); onNotify('Work order updated.') } catch (error) { onNotify(error.message) } }}><option>Open</option><option>In progress</option><option>Completed</option><option>Cancelled</option></select><button className="row-more" onClick={() => download(item.id)}>Download</button></div>)}</div></section></div>
}
...
function WorkspaceControls({ token, user, subscription, notifications, onNotify, onSubscriptionChanged, onNotificationsChanged }) {
  ...
  return <div>
    ...
    <section className="panel table-panel"><div className="table-header"><div><h2>Notifications</h2><span>{notifications.filter((item) => item.status === 'unread').length} unread</span></div></div><div className="data-table">{notifications.slice(0, 12).map((item) => <div className="data-row" key={item.id}><span><strong>{item.title}</strong><small>{item.detail}</small></span><span>{item.severity}</span><span>{item.status}</span>{item.status === 'unread' ? <button className="row-more" onClick={async () => { try { const updated = await updateNotification(token, item.id, 'read'); onNotificationsChanged((current) => current.map((entry) => entry.id === updated.id ? updated : entry)) } catch (error) { onNotify(error.message) } }}>Mark read</button> : <span />}</div>)}</div></section>
  </div>
}

function Costs({ token, expenses, vehicles: fleet, onNotify, onExpenseCreated }) {
